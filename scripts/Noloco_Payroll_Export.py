...
def create_pay_calculations_sheet(wb, df, styles, company_name, period_text, hourly_rate=None, min_wage_rate=8.50):
    """
    Sheet 3: Pay calculations with MANUAL commission entry and automatic logic
    ...
    Args:
        ...
        hourly_rate: Default hourly rate if not in user data
    ...
    ws[f'A{current_row}'] = "1. For COMMISSION employees: Enter total commission $ in the GRAY 'Commission $' column"
    ws[f'A{current_row}'].font = Font(italic=True, size=9)
    current_row += 1
    
    ws[f'A{current_row}'] = f"2. System compares commission vs minimum wage guarantee (${min_wage_rate}/hr × hours worked)"
    ws[f'A{current_row}'].font = Font(italic=True, size=9)
    current_row += 1
    
    ws[f'A{current_row}'] = "3. Commission employees receive whichever is HIGHER (commission or minimum wage)"
    ws[f'A{current_row}'].font = Font(italic=True, size=9)
    current_row += 1
    
    ws[f'A{current_row}'] = "4. HOURLY employees are paid at their hourly rate (commission column is ignored)"
    ws[f'A{current_row}'].font = Font(italic=True, size=9)
    current_row += 1
    
    ws[f'A{current_row}'] = "5. Leave commission blank if employee earned $0 - they'll automatically get minimum wage"
    ws[f'A{current_row}'].font = Font(italic=True, size=9)
    current_row += 2
    ...
    for employee_id in df[employee_col].unique():
        ...
        # Minimum Wage Guarantee (automatic calculation: hours × $8.50)
        cell = ws.cell(row=current_row, column=col)
